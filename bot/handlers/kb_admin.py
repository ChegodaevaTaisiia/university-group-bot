...
async def kb_refresh(message: Message):
    ...
    from bot.services.kb_import.university_site import KB_SOURCES, refresh_from_site

    if not KB_SOURCES:
        await message.answer(
            "Источники не настроены. Пришли ссылку на страницу преподавателей/кафедр — "
            "добавлю парсер. Пока наполняй базу вручную через /kb_add."
        )
        return
    n = await refresh_from_site(get_sessionmaker())
    await message.answer(f"Обновила: {n} записей.")
